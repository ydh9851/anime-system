<template>
  <div class="app-container">
    ...
    <el-form :model="form" ref="form" label-width="100px" v-loading="formLoading" :rules="rules">
      <el-form-item label="动漫名字："  required>
        <el-input v-model="form.videoName"></el-input>
      </el-form-item>
      <el-form-item label="动漫分类：" prop="videoCategory" required>
        <el-select v-model="form.videoCategory" placeholder="动漫分类">
          <el-option v-for="item in categoryEnum" :key="item.key" :value="item.key" :label="item.value"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :key="index" :label="'动漫标签'+(index+1)+'：'" required v-for="(titleItem,index) in form.videoTagList">
...
export default {
  ...
  methods: {
    ...
    removeTitleItem (titleItem) {
      const idx = this.form.videoTagList.indexOf(titleItem)
      if (idx > -1) {
        this.form.videoTagList.splice(idx, 1)
      }
    },
    resetForm () {
