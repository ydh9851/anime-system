...
class Movie_KNN_recommender:
    def __init__(self, mode=0):
        self.index = pd.read_csv(os.path.join(DATA_DIR, 'personal', 'anime_titles.csv'))
        self.reader = Reader()
...
if __name__ == "__main__":
    # 演示代码：演示电影可能不在当前评分数据集中，优雅跳过而不是崩溃
    try:
        test.algo.trainset.to_inner_iid(122922)
    except ValueError:
        print("演示电影 122922 不在当前评分数据集中，跳过演示；请通过 recommend_api.py --algo movie_knn 调用")
    else:
