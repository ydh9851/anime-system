...
成功输出（单行 JSON，stdout）:
    {"algo": "demographic", "movies": [{"title": "...", "movieId": 1, "score": 8.06}, ...]}
    - movies[i].title    : 动漫标题（string）
    - movies[i].movieId  : 动漫 MAL id（int，即 t_video_info.video_id）
...
注意:
    ...
    - naive 三算法基于 anime_5000.csv，数据统一从 data/recommendation 读取，
...
# ---------------------------------------------------------------------------
# 数据映射：MovieLens movieId -> MAL id -> 标题
# ---------------------------------------------------------------------------
_LINKS = None          # {MovieLens movieId(int) -> MAL id(int)}
_MOVIES_INDEX = None   # {MovieLens movieId(int) -> MovieLens title(str)}
_ANIME_TITLES = None   # {MAL id(int) -> anime_5000 title(str)}
...
def get_links():
    """MovieLens movieId -> MAL id。MAL id 缺失（NaN）的动漫被剔除。"""
    global _LINKS
    if _LINKS is None:
        import pandas as pd
...
def get_movies_index():
    ...
    if _MOVIES_INDEX is None:
        import pandas as pd
        movies = pd.read_csv(os.path.join(PERSONAL_DATA_DIR, "anime_titles.csv"))
        _MOVIES_INDEX = dict(zip(movies["movieId"], movies["title"]))
    return _MOVIES_INDEX


def get_tmdb_titles():
    """MAL id -> 动漫标题（来自 data/anime_5000.csv）。保留函数名以兼容旧代码。"""
    global _ANIME_TITLES
    if _ANIME_TITLES is None:
        import pandas as pd
        anime = pd.read_csv(
            os.path.join(DATA_DIR, "anime_5000.csv"), usecols=["id", "title"]
        )
        anime = anime.dropna(subset=["id", "title"])
        anime["id"] = anime["id"].astype(int)
        _ANIME_TITLES = dict(zip(anime["id"], anime["title"]))
    return _ANIME_TITLES
...
# ---------------------------------------------------------------------------
# naive 三算法（基于 anime_5000.csv，返回 TMDB id + title）
# ---------------------------------------------------------------------------
...
def run_usr_keywords(user, keywords, top):
    ...
    # 标题是 anime_5000 的动漫标题 -> 反向映射 animeId
    tmdb_titles = get_tmdb_titles()
    norm_map = {}
