...
SYSTEM_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # 系统工程根目录 anime-system/
...
def analyze_data(df):
    ...
    print("\n" + "=" * 50)
    print("4. 目标变量（成员数revenue）分布")
    print("=" * 50)
    print(f"成员数均值：{df['revenue'].mean():.2f}")
    print(f"成员数中位数：{df['revenue'].median():.2f}")
    print(f"成员数最大值：{df['revenue'].max():.2f}")
    print(f"成员数最小值：{df['revenue'].min():.2f}")

    print("\n" + "=" * 50)
    print("5. 动漫语言（original_language）分布")
    print("=" * 50)
    lang_count = df['original_language'].value_counts()
    print(lang_count.head(10))  # 输出前10种语言的动漫数量


# 3. 数据预处理代码（缺失值处理、特征编码、特征筛选）
def preprocess_data(df):
    """数据预处理：处理缺失值、编码类别特征、筛选有效特征"""
    # 3.1 筛选核心特征（排除无关列，保留对成员数有影响的特征）
    useful_features = [
    ...
    y = df_processed['revenue']  # 目标变量（成员数）
...
def plot_lang_revenue_boxplot(X_test, y_test, y_pred, lang_encoder, lang_mapping_df):
    ...
    # 6.2 合并真实成员数与预测成员数（可选：可同时展示真实/预测分布）
    plot_df = X_test_with_lang[['original_language']].copy()
    plot_df['revenue_true'] = y_test.values  # 真实成员数
    plot_df['revenue_pred'] = y_pred  # 预测成员数（可选展示）
    ...
    sns.boxplot(
        ...
        data=plot_df,
        palette='Set2',  # 配色方案
        showfliers=True  # 显示异常值（成员数中的高收入动漫）
    )

    # 6.5 图表美化（与目标图标题、轴标签一致）
    plt.title('Revenue by original language\'s animes', fontsize=16, pad=20)
    plt.xlabel('Original language', fontsize=12, labelpad=10)
    plt.ylabel('Revenue', fontsize=12, labelpad=10)
    plt.yscale('symlog')  # 对称对数刻度（适配成员数长尾分布，避免高收入动漫掩盖其他分布）
