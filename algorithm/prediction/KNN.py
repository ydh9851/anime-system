...
def preprocess_data(valid_df):
    ...
    # 假设原始popularity范围在0-400（常见动漫数据集范围），无需除以1e8，直接使用原始值或除以100
    X = valid_df[['popularity']] / 100  # 轻微缩放，让x轴范围匹配图2（0-4）
    y = valid_df['revenue'] / 1e9  # 成员数仍保持1e9单位（与图2一致）
...
def visualize(X, y, model, X_scaled):
    ...
    # 坐标轴设置
    plt.xlabel('动漫受欢迎程度', fontsize=12)
    plt.ylabel('成员数', fontsize=12)
    plt.title('动漫受欢迎程度和成员数之间的联系', fontsize=14)
