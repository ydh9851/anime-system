...
def preprocess_data(valid_df):
    # 特征与目标变量（转换单位：预算→千万美元，成员数→亿美元）
    X = valid_df[['budget']] / 1e8  # 预算单位：千万美元
    y = valid_df['revenue'] / 1e9  # 成员数单位：亿美元
...
def train_model(X_train, y_train):
    ...
    print(f"\n线性回归方程：成员数 = {model.coef_[0]:.2f}×预算 + {model.intercept_:.2f}")
    return model
...
def visualize(X, y, model, y_pred, X_test):
    ...
    plt.xlabel('动漫预算（千万美元）', fontsize=12)
    plt.ylabel('动漫成员数（亿美元）', fontsize=12)
    plt.title('动漫预算与成员数关系及线性回归预测', fontsize=14)
    plt.legend()
    plt.grid(alpha=0.3)
