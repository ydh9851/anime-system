...
特征与 LightGBM.py 完全一致：
    original_language / budget(集数) / popularity(成员数) / runtime / status(类型)  ->  revenue(成员数，热度指标)
...
def main():
    ...
    try:
        ...
        parser.add_argument("--popularity", type=float, default=10000.0, help="成员数 popularity")
        parser.add_argument("--runtime", type=int, default=24, help="单集时长（分钟）")
        parser.add_argument("--language", default="ja", help="原始语言代码，如 ja/en")
        parser.add_argument("--status", default="TV", help="类型，如 TV/Anime/OVA")
